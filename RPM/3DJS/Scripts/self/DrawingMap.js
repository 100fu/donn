...
sys.draw = {
            ...
            if (data.length > 0) {
                ...
                $.each(data, function (i, item) {
                    if (item.Position != null) {
                        ...
                        if (strList.length > 1) {
                            var ps = { X: strList[0], Y: strList[1], Z: strList[2] };
                            var psNew = new Point3D_CM(ps);  //完成坐标偏移
                            var dd = sys.draw.initPoint3d(psNew, color);
                            dd.objType = 'sta';
                            dd.origPosition = ps;
                            dd.staId = item.Id;
                            dd.showname = item.Name;
                            dd.showcode = item.Code;
                            if (ismap3D) {
                                sys.baseNew3D.addStationList(item);
    ...
    initRegWLConfig: function () {
        //$.getJSON(fileParam.sysRegWL_path + "?random=" + new Date().getTime(), function (data) {
        //    if (data == '')
        //        return;
        //    var jsonstr = JSON.stringify(data);
        //    // 存储json字符串  
        //    window.localStorage.setItem('alarmRegion', jsonstr);
        //});       
    ...
    initRegDMConfig: function () {
        //$.getJSON(fileParam.sysRegDM_path + "?random=" + new Date().getTime(), function (data) {
        //    if (data == '')
        //        return;
        //    //var jsonstr = JSON.stringify(sysConfig);
        //    var jsonstr = JSON.stringify(data);
        //    // 存储json字符串  
        //    window.localStorage.setItem('rollcallRegion', jsonstr);
        //});
    ...
    initSysConfig: function () {
        //GetSysOption
        //$.getJSON(fileParam.sysConfig_path + "?random=" + new Date().getTime(), function (data) {
        //    if (data == '')
        //        return;
        //    // tranX = 650, tranY = 1680;
        //    var obj = data;
        ...
        //    //tranX = -1128, tranY = 799;
        //    // sys.draw.saveLocStorSys();
        //});
        ...
        clientMode.post(urlPath, strJson, function (data) {
            if (data.length > 0) {
                ...
                if (obj.floor === '首层') {
                    ...
                    sysConfig.mapHeight = obj.mapHeight;
                    GridSize = sysConfig.mapWidth + 100;
                    sysConfig.floor = obj.floor;

                    SysConfingList.push(sysConfig);
                }
            }
            if (allbackfun !== '') {
    ...
    testLoadShape: function () {
        ...
        // splinepts.push( new THREE.Vector2( 280.9962759416929, 111.10295518915176 ) );
        // splinepts.push( new THREE.Vector2( 235.97899410141753, 133.34413150527337 ) );
        // splinepts.push( new THREE.Vector2( 214.29618292562486, 118.16616368221851 ) );
        // splinepts.push( new THREE.Vector2( 226.8766131595159, 105.86726442947896 ) );

        // var splineShape = new THREE.Shape();
        // splineShape.moveTo(226.8766131595159, 105.86726442947896  );
        // splineShape.splineThru( splinepts );
        // var geometry = new THREE.ShapeBufferGeometry( splineShape );
        // var color=0x00CC00;

        // var mesh = new THREE.Mesh( geometry, new THREE.MeshPhongMaterial( { color: color, side: THREE.DoubleSide,opacity:0.8 } ) );
        // scene2D.add(mesh);
    ...
    getRegsInfo: function (key) {
        ...
        clientMode.post(urlPath, strJson, function (data) {
            for (var i = 0; i < data.length; i++) {
                ...
                if (data[i].DrawJson != "" && data[i].DrawJson != "OL")
    ...
    getRegsCallback: function (key, callback) {
    ...
    initFloorMapJPG: function () {
        ...
        var mapInfo = getLayerByName(curFloor); //SysConfingList[index];
        var fileMPath = fileParam.sysMap_path + mapInfo.mapName;
        //sysConfig
        //var sphere = sys.draw.createMesh(new THREE.PlaneBufferGeometry(2256, 1598), fileMPath);

        var sphere = sys.draw.createMesh(new THREE.PlaneBufferGeometry(mapInfo.mapWidth, mapInfo.mapHeight), fileMPath);
    ...
    //sys.draw.initPoint3d(psNew, color);
                ...
                $.each(data, function (i, item) {
                    if (item.Position !== null && item.Floor === floor) {
                        ...
                        if (strList.length > 1) {
                            ...
                            let sta = sys.draw.initPoint3d(psNew, color);
                            sta.objType = 'sta';
                            // sta.origPosition = ps;
                            sta.staId = item.Id;
                            sta.floor = floor;
    ...
    loadStation3D: function (planMap3D, floor) {
        let color = '#A020F0';
        let urlPath = '/Graphic/GetStaInfoList';
        let selfPlan = planMap3D;
        let selfFloor = floor;
